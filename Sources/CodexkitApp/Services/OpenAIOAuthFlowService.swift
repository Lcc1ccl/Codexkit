...
struct OpenAIOAuthFlowService {
    ...
    func completeFlow(
        ...
        if let returnedState = parsed.state,
           returnedState != flow.expectedState {
            NSLog(
                "codexkit OAuth state mismatch on completion: expected=%@ returned=%@; attempting PKCE exchange anyway",
                flow.expectedState,
                returnedState
            )
        }
    ...
    func refreshAccount(_ account: TokenAccount) async throws -> TokenAccount {
        ...
        let refreshedAccessClaims = AccountBuilder.decodeJWT(tokens.accessToken)
        let refreshedPlanType = (refreshedAccessClaims["https://api.openai.com/auth"] as? [String: Any])?["chatgpt_plan_type"] as? String
        if let refreshedPlanType = refreshedPlanType?.trimmingCharacters(in: .whitespacesAndNewlines),
           refreshedPlanType.isEmpty == false {
            refreshed.planType = refreshedPlanType
        } else if refreshed.planType == "free" {
            refreshed.planType = account.planType
        }
