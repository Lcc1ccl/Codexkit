...
struct CodexBarOAuthAccountService {
    ...
    func importAccount(_ account: TokenAccount, activate: Bool) throws -> OAuthAccountMutationResult {
        ...
        let intent: CodexSyncIntent? = activate && previousConfig.desktop.cliProxyAPI.enabled ? .disableAPIServiceAndSwitch : nil
        if intent != nil {
            config.desktop.cliProxyAPI.enabled = false
        }

        try self.persist(
            config: config,
            previousConfig: previousConfig,
            synchronizeCodex: result.syncCodex,
            intent: intent
        )
        if activate {
    ...
    func importAccounts(_ accounts: [TokenAccount], activeAccountID: String?) throws -> OAuthAccountBatchImportResult {
        ...
        let syncIntent: CodexSyncIntent? = activeAccountID != nil && previousConfig.desktop.cliProxyAPI.enabled
            ? .disableAPIServiceAndSwitch
            : nil
        if syncIntent != nil {
            config.desktop.cliProxyAPI.enabled = false
        }
    ...
    private func persist(
        ...
        do {
            if synchronizeCodex {
                if let intent {
                    try self.syncService.synchronize(config: finalConfig, intent: intent)
                } else {
            ...
            if synchronizeCodex {
                _ = try? self.syncService.restoreNativeConfiguration(desktopSettings: finalConfig.desktop)
            }
