...
final class CodexBarConfigStore {
    ...
    private func migrateFromLegacy() throws -> CodexBarConfig {
        ...
        let global = CodexBarGlobalSettings(
            ...
            serviceTier: toml.serviceTier.flatMap(CodexBarConfigStore.parseServiceTier(_:))
        )
    ...
    func reconcileAuthJSON(
        ...
        guard let snapshot = self.authJSONSnapshot(from: self.readAuthJSON(for: original)) else {
    ...
    func oauthAccountID(fromAuthJSONData data: Data?) -> String? {
        guard let data,
              let auth = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let snapshot = self.authJSONSnapshot(from: auth) else {
            return nil
        }
        return self.accountFromAuthSnapshot(snapshot).id
    }
    ...
    private func readLegacyToml(for config: CodexBarConfig? = nil) -> LegacyCodexTomlSnapshot {
        let target = config.map { CodexPaths.primaryNativeTarget(for: $0.desktop) } ?? CodexPaths.globalNativeTarget()
        guard let text = try? String(contentsOf: target.configTomlURL, encoding: .utf8) else {
    ...
    private static func parseServiceTier(_ value: String) -> CodexBarServiceTier? {
        let normalized = value.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        switch normalized {
        case CodexBarServiceTier.fast.rawValue:
            return .fast
        case CodexBarServiceTier.standard.rawValue:
            return .standard
        default:
            return nil
        }
    ...
    private func matchOpenAIBaseURL(in text: String) -> String? {
        ...
        return self.matchBaseURLInProviderBlock(in: text, key: "OpenAI")
            ...
            ?? self.matchBaseURLInProviderBlock(in: text, key: "custom")
    }
    ...
    private func readAuthJSON(for config: CodexBarConfig? = nil) -> [String: Any] {
        let target = config.map { CodexPaths.primaryNativeTarget(for: $0.desktop) } ?? CodexPaths.globalNativeTarget()
        guard let data = try? Data(contentsOf: target.authURL),
