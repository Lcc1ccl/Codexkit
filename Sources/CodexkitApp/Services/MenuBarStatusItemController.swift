...
extension Notification.Name {
    static let codexkitRequestCloseStatusItemMenu = Notification.Name("lzl.codexkit.status-item-menu.close")
    static let codexkitStatusItemMeasuredHeightDidChange = Notification.Name("lzl.codexkit.status-item-menu.height-changed")
    static let codexkitRequestStatusItemLayoutRefresh = Notification.Name("lzl.codexkit.status-item-menu.layout-refresh")
}
...
final class MenuBarStatusItemController: NSObject, NSPopoverDelegate {
    ...
    private var onNextPopoverDidClose: (() -> Void)?
    ...
    func dismissPopoverIfNeeded(afterClose action: (() -> Void)? = nil) {
        if self.popover.isShown {
            self.onNextPopoverDidClose = action
            self.closePopover()
            return
        }
    ...
    private func bindState() {
        ...
        NotificationCenter.default.publisher(for: .codexkitRequestCloseStatusItemMenu)
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in
                self?.closePopover()
            }
        ...
        NotificationCenter.default.publisher(for: .codexkitStatusItemMeasuredHeightDidChange)
            .receive(on: RunLoop.main)
        ...
        NotificationCenter.default.publisher(for: .codexkitRequestStatusItemLayoutRefresh)
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in
    ...
    private func updateAppearance() {
        ...
        let presentation = MenuBarStatusItemPresentation.make(
            ...
            menuBarDisplay: TokenStore.shared.config.openAI.menuBarDisplay,
            updateAvailable: UpdateCoordinator.shared.pendingAvailability != nil,
            apiServiceEnabled: TokenStore.shared.config.desktop.cliProxyAPI.enabled,
            apiServiceMemberAccountIDs: TokenStore.shared.config.desktop.cliProxyAPI.memberAccountIDs,
    ...
    private func closePopover(_ sender: AnyObject? = nil) {
        guard self.popover.isShown else { return }
        DetachedWindowPresenter.shared.closeHoverPanels()
    ...
    func popoverDidClose(_ notification: Notification) {
        ...
        let action = self.onNextPopoverDidClose
        self.onNextPopoverDidClose = nil
        action?()
    }
}
