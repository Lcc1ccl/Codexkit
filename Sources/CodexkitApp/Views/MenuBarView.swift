...
enum MenuBarScrollViewConfiguration {
    static func apply(
        to scrollView: NSScrollView,
        idleScrollerAlpha: CGFloat
    ) {
        ...
        scrollView.verticalScroller?.alphaValue = idleScrollerAlpha
...
private final class AdaptiveMenuScrollHost: NSView {
    ...
    init(
        ...
        MenuBarScrollViewConfiguration.apply(
            to: self.scrollView,
            idleScrollerAlpha: self.idleScrollerAlpha
        )
...
struct MenuBarView: View {
    ...
    @State private var revealsAPIServiceSecret = false
    @State private var languageRefreshToken = false
    @State private var showsNativeRestoreConfirmation = false
    ...
    private var apiServiceFallbackBanner: OpenAIStatusBannerPresentation? {
        OpenAIAccountPresentation.apiServiceFallbackBanner(
            serviceability: self.store.apiServicePoolServiceability(now: self.now),
            hasServiceableFallbackCandidate: self.hasServiceableFallbackCandidate
        )
    }

    private var hasServiceableFallbackCandidate: Bool {
        if self.store.accounts.contains(where: \.isAvailableForNextUseRouting) {
            return true
        }
        return self.store.config.providers.contains { provider in
            provider.kind != .openAIOAuth && provider.activeAccount != nil
        }
    }
    ...
    var body: some View {
        ...
        .onChange(of: self.lastOpenAIManualSwitchResult) {
            self.requestStatusItemLayoutRefresh()
        }
        .confirmationDialog(
            L.apiServiceFallbackRestoreConfirmTitle,
            isPresented: self.$showsNativeRestoreConfirmation,
            titleVisibility: .visible
        ) {
            Button(L.apiServiceFallbackRestoreAction, role: .destructive) {
                self.performExplicitNativeRestore()
            }
            Button(L.cancel, role: .cancel) {}
        } message: {
            Text(L.apiServiceFallbackRestoreConfirmMessage)
        }
        .onChange(of: self.isAPIServiceConfigComplete) {
            self.clearResolvedAPIServicePreflightWarning()
        }
        .onChange(of: self.isAPIServiceRuntimeReadyForRouting) {
            self.clearResolvedAPIServicePreflightWarning()
        }
        .onAppear {
    ...
    private var menuContentStack: some View {
        let _ = self.languageRefreshToken
        return VStack(alignment: .leading, spacing: 0) {
            HStack {
                ...
                .help(L.menuOpenAIHeaderEmptyHint)
            ...
            HStack(spacing: 8) {
                ...
                Menu {
                    Button {
                        self.setLanguageOverride(nil)
                    } label: {
                        Label(
                            L.menuLanguageAuto,
                            systemImage: L.languageOverride == nil ? "checkmark" : ""
                        )
                    }

                    Button {
                        self.setLanguageOverride(true)
                    } label: {
                        Label(
                            L.menuLanguageChinese,
                            systemImage: L.languageOverride == true ? "checkmark" : ""
                        )
                    }

                    Button {
                        self.setLanguageOverride(false)
                    } label: {
                        Label(
                            L.menuLanguageEnglish,
                            systemImage: L.languageOverride == false ? "checkmark" : ""
                        )
                    }
                } label: {
                    Image(systemName: "globe")
                        ...
                        .foregroundStyle(self.languageMenuIconColor)
                }
                .menuStyle(.borderlessButton)
                .tint(self.languageMenuIconColor)
                .help(L.menuLanguageToggle)
                ...
                Button {
                    ...
                    Image(systemName: "gearshape")
                        .font(.system(size: 12))
                        .symbolRenderingMode(.monochrome)
                        .foregroundStyle(self.footerToolbarIconColor)
                }
                .buttonStyle(.borderless)
                .help(L.settings)
    ...
    private func updateAvailableBanner(availability: AppUpdateAvailability) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                .fill(self.apiServiceCardBackgroundColor)
        )
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(self.apiServiceCardBorderColor, lineWidth: 1)
        )
    }

    private var apiServiceSummarySection: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text(L.menuAPIServiceTitle)
                    .font(.system(size: 11, weight: .medium))

                Spacer(minLength: 0)

                Button {
                    self.openSettingsWindow(page: .apiServiceDashboard)
                } label: {
                    Image(systemName: "chart.xyaxis.line")
                        .font(.system(size: 10, weight: .medium))
                }
                .buttonStyle(.borderless)
                .help(L.settingsAPIServiceDashboardPageTitle)

                Button {
                    self.openSettingsWindow(page: .apiServiceLogs)
                } label: {
                    Image(systemName: "doc.text.magnifyingglass")
                        .font(.system(size: 10, weight: .medium))
                }
                .buttonStyle(.borderless)
                .help(L.settingsAPIServiceLogsPageTitle)

                HStack(spacing: 6) {
                    Circle()
                        .fill(self.apiServiceStatusDotColor)
                        .frame(width: 7, height: 7)

                    Text(self.apiServiceStatusLabel)
                        .font(.system(size: 10, weight: .medium))
                }
                .padding(.horizontal, 9)
                .padding(.vertical, 3)
                .background(self.apiServiceStatusTagBackground)
                .foregroundColor(self.apiServiceStatusTagForeground)
                .clipShape(Capsule())
            }

            self.apiServiceInfoRow(
                title: L.menuAPIServiceAddressLabel,
                value: self.apiServiceEndpointText
            ) {
                Button {
                    let value = self.apiServiceEndpointText
            ...
            HStack(alignment: .center, spacing: 18) {
                self.apiServiceToggleControl(
                    ...
                    isOn: Binding(
                        get: { self.isAPIServiceRuntimeActive },
                        set: { _ in self.toggleAPIServiceRuntime() }
                    )
                )
                .frame(maxWidth: .infinity, alignment: .leading)

                self.apiServiceToggleControl(
                    title: L.menuAPIServiceConfigToggleTitle,
                    isOn: Binding(
                        get: { self.store.config.desktop.cliProxyAPI.enabled },
                        set: { _ in self.toggleAPIServiceEnabled() }
                    )
                )
    ...
    private var apiServiceStatusLabel: String {
        self.isAPIServiceRuntimeActive ? L.menuAPIServiceRunningState : L.menuAPIServiceStoppedState
    }

    private var apiServiceStatusDotColor: Color {
        self.isAPIServiceRuntimeActive
            ? Color(red: 0.34, green: 0.63, blue: 0.48)
            : .secondary
    }

    private var apiServiceStatusTagBackground: Color {
        self.apiServiceStatusDotColor.opacity(self.isAPIServiceRuntimeActive ? 0.16 : 0.1)
    }

    private var apiServiceStatusTagForeground: Color {
        self.apiServiceStatusDotColor
    }

    private var apiServiceCardBackgroundColor: Color {
        Color(red: 0.12, green: 0.12, blue: 0.14).opacity(0.08)
    }

    private var apiServiceCardBorderColor: Color {
        if self.store.config.desktop.cliProxyAPI.enabled || self.isAPIServiceRuntimeActive {
            return Color.accentColor.opacity(0.18)
        }
        return Color.primary.opacity(0.08)
    }

    private var isAPIServiceRuntimeActive: Bool {
        self.store.cliProxyAPIState.runtimeProcessLikelyActive
    }

    private var apiServiceToggleRefreshKey: String {
        "\(self.store.config.desktop.cliProxyAPI.enabled)-\(self.store.cliProxyAPIState.status.rawValue)-\(self.store.cliProxyAPIState.pid ?? -1)"
    }

    private var isAPIServiceRuntimeReadyForRouting: Bool {
        self.store.cliProxyAPIState.status == .running
    }

    private var isAPIServiceConfigComplete: Bool {
        let settings = self.store.config.desktop.cliProxyAPI
        return settings.memberAccountIDs.isEmpty == false
    }

    private var apiServiceEndpointText: String {
        "\(self.store.cliProxyAPIState.config.baseURL.absoluteString)/v1"
    }

    private var footerToolbarIconColor: Color {
        Color.secondary.opacity(0.78)
    }

    private var languageMenuIconColor: Color {
    ...
    private func apiServiceInfoRow<Actions: View>(
        title: String,
        value: String,
            ...
            if store.accounts.isEmpty {
                VStack(alignment: .leading, spacing: 4) {
                    Text("No OpenAI account added.")
                        .font(.system(size: 11, weight: .medium))
                    Text(L.menuOpenAIHeaderEmptyHint)
                        .font(.system(size: 10))
                        .foregroundColor(.secondary)
                }
                .padding(.horizontal, 10)
        ...
        if providerCount > 0 {
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 8) {
                    ...
                    Button {
                        self.openSettingsWindow(page: .provider)
                    } label: {
                        Image(systemName: "arrow.up.right.square")
                            .font(.system(size: 10, weight: .medium))
                    }
                    .buttonStyle(.borderless)
                    .help(L.settingsProviderPageTitle)
                ...
                if isProvidersExpanded {
                    ForEach(store.customProviders) { provider in
                        CompatibleProviderRowView(
                            ...
                            openProviderConfigurationWindow(provider: provider)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    ForEach(group.accounts) { account in
                        let rowState = OpenAIAccountPresentation.rowState(
                            for: account,
                            summary: self.runningThreadSummary,
                            forceUseAction: self.store.config.desktop.cliProxyAPI.enabled
                        )
                        AccountRowView(
                            account: account,
                            rowState: rowState,
    ...
    private func reportMeasuredMenuHeight(_ height: CGFloat) {
        NotificationCenter.default.post(
            name: .codexkitStatusItemMeasuredHeightDidChange,
            object: nil,
    ...
    private func requestStatusItemLayoutRefresh() {
        DispatchQueue.main.async {
            NotificationCenter.default.post(
                name: .codexkitRequestStatusItemLayoutRefresh,
                object: nil
            )
        }
    }
    ...
    private func openSettingsWindow(page: SettingsPage = .accounts) {
        MenuBarStatusItemController.shared.dismissPopoverIfNeeded {
            SettingsWindowLauncher.shared.show(initialPage: page)
        }
    }
    ...
    private func toggleAPIServiceEnabled() {
        ...
        guard self.isAPIServiceConfigComplete else {
            self.presentAPIServicePreflightAlert(L.menuAPIServiceSetupRequiredMessage)
            return
        }

        guard self.isAPIServiceRuntimeReadyForRouting else {
            self.showError = L.menuAPIServiceStartRequiredMessage
            return
        }

        Task {
            do {
                let message = try await self.store.enableAPIServiceRoutingFromMenu()
    ...
    private func clearResolvedAPIServicePreflightWarning() {
        guard let currentMessage = self.showError else { return }
        let legacySetupMessage = L.zh ? "请先在设置中完成API服务配置" : "Finish API Service setup in Settings first."

        if [L.menuAPIServiceSetupRequiredMessage, legacySetupMessage].contains(currentMessage),
           self.isAPIServiceConfigComplete {
            self.showError = nil
        }

        if currentMessage == L.menuAPIServiceStartRequiredMessage,
           self.isAPIServiceConfigComplete,
           self.isAPIServiceRuntimeReadyForRouting {
            self.showError = nil
        }
    }

    private func presentAPIServicePreflightAlert(_ message: String) {
        self.showError = message
    }

    private func handleAPIServiceFallbackBannerAction(_ banner: OpenAIStatusBannerPresentation) {
        if banner.actionTitle == L.apiServiceFallbackDisableAction {
            self.toggleAPIServiceEnabled()
            return
        }
        if banner.actionTitle == L.apiServiceFallbackRestoreAction {
            self.showsNativeRestoreConfirmation = true
            return
        }
        self.openSettingsWindow(page: .apiService)
    }

    private func performExplicitNativeRestore() {
        do {
            try self.store.disableAPIServiceRoutingAndRestoreDirect()
            self.refreshRunningThreadAttribution()
            self.showError = L.apiServiceFallbackRestoreCompleted
        } catch {
            self.showError = error.localizedDescription
        }
    }

    private func setLanguageOverride(_ override: Bool?) {
        L.languageOverride = override
        self.languageRefreshToken.toggle()
    }
    ...
    private func localizedRuntimeStatus(_ status: CLIProxyAPIServiceState.RuntimeStatus) -> String {
        switch status {
        case .stopped:
            return "Stopped"
        case .starting:
    ...
    private func openProviderConfigurationWindow(provider: CodexBarProvider) {
        ...
        case .openAIOAuth:
            self.openSettingsWindow(page: .accounts)
        }
    }
    ...
    private func refresh(force: Bool = true, announceResult: Bool = false) async {
        if self.store.config.desktop.cliProxyAPI.enabled {
            await CLIProxyAPIRuntimeController.shared.refreshQuotaSnapshot(trigger: "menubar-all")
    ...
    private func refreshAccount(_ account: TokenAccount, announceResult: Bool) async {
        if self.store.config.desktop.cliProxyAPI.enabled {
            await CLIProxyAPIRuntimeController.shared.refreshQuotaSnapshot(trigger: "menubar-account-\(account.accountId)")
            if announceResult, let lastError = self.store.cliProxyAPIState.lastError, lastError.isEmpty == false {
    ...
    private func requestCloseStatusItemMenu() {
        NotificationCenter.default.post(name: .codexkitRequestCloseStatusItemMenu, object: nil)
    }
    ...
    private func refreshImportedAccounts(accountIDs: [String]) {
        if self.store.config.desktop.cliProxyAPI.enabled {
            Task {
                await CLIProxyAPIRuntimeController.shared.refreshQuotaSnapshot(trigger: "menubar-imported")
            }
            return
        }
...
private struct EditableCompatibleProviderAccountDraft: Identifiable {
    var id: String
    var label: String
    var apiKey: String
}

private struct EditCompatibleProviderAccountRow: View {
    @Binding var account: EditableCompatibleProviderAccountDraft
    @Binding var activeAccountID: String?
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                TextField("Account label", text: self.$account.label)
                Button {
                    self.activeAccountID = accountID
                } label: {
                    Image(systemName: self.activeAccountID == accountID ? "checkmark.circle.fill" : "circle")
                        .font(.system(size: 12))
                }
                .buttonStyle(.borderless)
            ...
            SecureField("API key", text: self.$account.apiKey)
        }
        .padding(10)
        .background(
            RoundedRectangle(cornerRadius: 8)
...
struct EditCompatibleProviderSheet: View {
    let provider: CodexBarProvider
    let onSave: (String, String, [CodexBarProviderAccount], String?) -> Void
    let onCancel: () -> Void

    @State private var label: String
    @State private var baseURL: String
    @State private var activeAccountID: String?
    @State private var accounts: [EditableCompatibleProviderAccountDraft]

    init(
        provider: CodexBarProvider,
        onSave: @escaping (String, String, [CodexBarProviderAccount], String?) -> Void,
        onCancel: @escaping () -> Void
    ) {
        ...
        self._activeAccountID = State(initialValue: provider.activeAccountId ?? provider.accounts.first?.id)
        self._accounts = State(
            initialValue: provider.accounts.map {
                EditableCompatibleProviderAccountDraft(
                    id: $0.id,
                    label: $0.label,
                    apiKey: $0.apiKey ?? ""
                )
    ...
    private var canSave: Bool {
        self.label.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false &&
            self.baseURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false &&
            self.accounts.isEmpty == false &&
            self.accounts.allSatisfy {
    ...
    private func removeCompatibleProviderAccount(_ accountID: String) {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextField("Provider name", text: self.$label)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    ForEach(self.accounts.indices, id: \.self) { index in
                        EditCompatibleProviderAccountRow(
                            account: self.$accounts[index],
                            activeAccountID: self.$activeAccountID
                        ) { accountID in
                            self.removeCompatibleProviderAccount(accountID)
                        }
                    }
                }
            }
            ...
            Button {
                let newAccount = EditableCompatibleProviderAccountDraft(
                    id: UUID().uuidString,
                    label: "Account \(self.accounts.count + 1)",
                    apiKey: ""
                )
                    ...
                    self.onSave(
                        ...
                        self.accounts.map {
                            CodexBarProviderAccount(
                                ...
                                apiKey: $0.apiKey.trimmingCharacters(in: .whitespacesAndNewlines),
                                addedAt: Date()
                            )
                        },
...
struct AddOpenRouterAccountSheet: View {
    let provider: CodexBarProvider
    @ObservedObject var store: TokenStore
...
struct EditOpenRouterModelSheet: View {
    let provider: CodexBarProvider
    @ObservedObject var store: TokenStore
...
struct OpenRouterProviderRowView: View {
    let provider: CodexBarProvider
