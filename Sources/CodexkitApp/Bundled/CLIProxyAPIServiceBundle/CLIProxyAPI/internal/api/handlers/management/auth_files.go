...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	if auth.Metadata != nil {
		if rawLocalAccountID, ok := auth.Metadata["codexkit_local_account_id"].(string); ok {
			if trimmed := strings.TrimSpace(rawLocalAccountID); trimmed != "" {
				entry["codexkit_local_account_id"] = trimmed
			}
		}
	}
...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	if errCreds != nil {
		log.WithError(errCreds).Error("gemini oauth credentials are unavailable")
		c.JSON(http.StatusFailedDependency, gin.H{"error": errCreds.Error()})
		return
	}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	authURL, errAuthURL := authSvc.BuildAuthURL(state, redirectURI)
	if errAuthURL != nil {
		log.WithError(errAuthURL).Error("failed to build antigravity auth URL")
		c.JSON(http.StatusFailedDependency, gin.H{"error": errAuthURL.Error()})
		return
	}
