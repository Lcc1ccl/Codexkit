...
final class CodexBarOAuthAccountServiceTests: CodexBarTestCase {
    ...
    private struct SaveOrderSyncService: CodexSynchronizing {
        let onSynchronize: (CodexBarConfig) throws -> Void

        func synchronize(config: CodexBarConfig) throws {
            try self.onSynchronize(config)
        }
    }
    ...
    func testImportActivatedAccountSynchronizesAuthAndConfig() throws {
        ...
        XCTAssertFalse(configText.contains("model_provider"))
        XCTAssertFalse(configText.contains("[model_providers.custom]"))
        XCTAssertFalse(configText.contains("model = "))
        XCTAssertFalse(configText.contains("review_model = "))
        XCTAssertFalse(configText.contains("model_reasoning_effort = "))
    }
    ...
    func testActivateAccountAutoDisablesAPIServiceBeforeSynchronizingDirectSelection() throws {
        ...
        _ = try service.activateAccount(accountID: second.accountId)

        XCTAssertEqual(syncService.lastIntent, .disableAPIServiceAndSwitch)
        XCTAssertFalse(syncService.lastConfig?.desktop.cliProxyAPI.enabled ?? true)
        XCTAssertEqual(syncService.lastConfig?.active.accountId, second.accountId)
    }

    func testActivateAccountSynchronizesBeforePersistingConfigStore() throws {
        let configStore = CodexBarConfigStore()
        let first = try self.makeOAuthAccount(accountID: "acct_first", email: "first@example.com", isActive: true)
        let second = try self.makeOAuthAccount(accountID: "acct_second", email: "second@example.com")
        let storedFirst = CodexBarProviderAccount.fromTokenAccount(first, existingID: first.accountId)
        ...
        let syncService = SaveOrderSyncService { synchronizedConfig in
            let persistedBeforeCommit = try configStore.load()
            XCTAssertEqual(persistedBeforeCommit.active.accountId, storedFirst.id)
            XCTAssertEqual(synchronizedConfig.active.accountId, storedSecond.id)
        }
        let service = CodexBarOAuthAccountService(
            configStore: configStore,
            syncService: syncService,
        ...
        let persistedAfterCommit = try configStore.load()
        XCTAssertEqual(persistedAfterCommit.active.accountId, storedSecond.id)
    }
