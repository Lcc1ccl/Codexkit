...
final class CodexBarConfigStoreTests: CodexBarTestCase {
    func testLoadOrMigrateDoesNotCreateGlobalCodexRootForSpecificPathsScope() throws {
        let store = CodexBarConfigStore()
        let scopedRoot = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: scopedRoot, withIntermediateDirectories: true)

        let account = CodexBarProviderAccount(
            id: "acct_scoped",
            kind: .oauthTokens,
            label: "scoped@example.com",
            email: "scoped@example.com",
            openAIAccountId: "acct_scoped",
            accessToken: "access-scoped",
            refreshToken: "refresh-scoped",
            idToken: "id-scoped"
        )
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: CodexPaths.barConfigURL.deletingLastPathComponent().path))
    }

    func testLoadOrMigrateReadsLegacyServiceTierFromNativeConfig() throws {
        ...
        XCTAssertEqual(loaded.global.serviceTier, .fast)
    }

    func testLoadOrMigrateReadsCustomModelProviderBaseURLFromNativeConfig() throws {
        try CodexPaths.ensureDirectories()
        try CodexPaths.writeSecureFile(
            Data(
                """
        ...
        try CodexPaths.writeSecureFile(
            Data(#"{"OPENAI_API_KEY":"client-key"}"#.utf8),
            to: CodexPaths.authURL
        )

        let loaded = try CodexBarConfigStore().loadOrMigrate()
        let provider = try XCTUnwrap(loaded.providers.first(where: { $0.kind == .openAICompatible }))

        XCTAssertEqual(provider.baseURL, "http://127.0.0.1:8317/v1")
    }

    func testLoadOrMigrateDoesNotDependOnProviderBlockNameOrWireAPI() throws {
        try CodexPaths.ensureDirectories()
        try CodexPaths.writeSecureFile(
            Data(
                """
        ...
        try CodexPaths.writeSecureFile(
            Data(#"{"OPENAI_API_KEY":"client-key","auth_mode":"apikey"}"#.utf8),
            to: CodexPaths.authURL
        )
        ...
        XCTAssertEqual(provider.baseURL, "https://compatible.example.com/v1")
    }

    func testLoadOrMigrateReadsAPIKeyAuthWithoutRequiringAuthMode() throws {
        try CodexPaths.ensureDirectories()
        try CodexPaths.writeSecureFile(
            Data(
                """
        ...
        XCTAssertEqual(account.apiKey, "client-key")
        XCTAssertEqual(provider.baseURL, "https://compatible.example.com/v1")
    }
    ...
    func testLoadOrMigrateDefaultsUnknownOpenAISettingsEnumValuesWithoutLosingOAuthAccounts() throws {
        ...
        openAI["menuBarDisplay"] = [
            "quotaVisibility": "future_menu_bar_quota_visibility",
            "apiServiceStatusVisibility": "future_menu_bar_api_service_status_visibility",
        ]
        ...
        XCTAssertEqual(loaded.openAI.menuBarDisplay.quotaVisibility, .both)
        XCTAssertEqual(loaded.openAI.menuBarDisplay.apiServiceStatusVisibility, .availableOverTotal)
    }

    func testLoadOrMigrateDefaultsCLIProxyAPIFreeRestrictionToTrueWhenMissing() throws {
        let store = CodexBarConfigStore()
        let config = CodexBarConfig(
            desktop: CodexBarDesktopSettings(
                cliProxyAPI: .init(
        ...
        let encoded = try encoder.encode(config)
        var object = try XCTUnwrap(JSONSerialization.jsonObject(with: encoded) as? [String: Any])
        var desktop = try XCTUnwrap(object["desktop"] as? [String: Any])
        var cliProxyAPI = try XCTUnwrap(desktop["cliProxyAPI"] as? [String: Any])
        cliProxyAPI.removeValue(forKey: "restrictFreeAccounts")
        desktop["cliProxyAPI"] = cliProxyAPI
        ...
        XCTAssertTrue(loaded.desktop.cliProxyAPI.restrictFreeAccounts)
    }

    func testLoadOrMigratePersistsCLIProxyAPIFreeRestrictionFalse() throws {
        let store = CodexBarConfigStore()
        try self.writeConfig(
            CodexBarConfig(
                desktop: CodexBarDesktopSettings(
        ...
        XCTAssertFalse(loaded.desktop.cliProxyAPI.restrictFreeAccounts)
    }

    func testLoadOrMigratePersistsPreAPIServiceDirectSelectionSnapshot() throws {
        let store = CodexBarConfigStore()
        try self.writeConfig(
            CodexBarConfig(
                desktop: CodexBarDesktopSettings(
                    cliProxyAPI: .init(
                        ...
                        preAPIServiceActiveProviderID: "compatible-provider",
                        preAPIServiceActiveAccountID: "acct-compatible",
                        memberAccountIDs: ["acct-alpha"]
                    )
                )
            )
        ...
        XCTAssertEqual(loaded.desktop.cliProxyAPI.preAPIServiceActiveProviderID, "compatible-provider")
        XCTAssertEqual(loaded.desktop.cliProxyAPI.preAPIServiceActiveAccountID, "acct-compatible")
    }

    func testLoadOrMigrateClearsLegacyCLIProxyAPIRepositoryRoot() throws {
        let store = CodexBarConfigStore()
        let config = CodexBarConfig(
            desktop: CodexBarDesktopSettings(
                cliProxyAPI: .init(
        ...
        var desktop = try XCTUnwrap(object["desktop"] as? [String: Any])
        var cliProxyAPI = try XCTUnwrap(desktop["cliProxyAPI"] as? [String: Any])
        cliProxyAPI["repositoryRootPath"] = "/tmp/legacy-cli-proxy"
        desktop["cliProxyAPI"] = cliProxyAPI
        object["desktop"] = desktop
        let mutated = try JSONSerialization.data(withJSONObject: object, options: [.prettyPrinted, .sortedKeys])
        try CodexPaths.writeSecureFile(mutated, to: CodexPaths.barConfigURL)
        ...
        XCTAssertNil(loaded.desktop.cliProxyAPI.repositoryRootPath)
        XCTAssertTrue(loaded.desktop.cliProxyAPI.enabled)
        XCTAssertEqual(loaded.desktop.cliProxyAPI.managementSecretKey, "secret")
        XCTAssertEqual(loaded.desktop.cliProxyAPI.memberAccountIDs, ["acct-alpha"])
    }
    ...
    func testOAuthAccountIDFromAuthJSONDataReturnsMatchingAccountIDForManagedOAuthPayload() throws {
        let store = CodexBarConfigStore()
        let account = try self.makeOAuthAccount(
            accountID: "acct_snapshot_helper",
            email: "snapshot-helper@example.com"
        )
        ...
        XCTAssertEqual(store.oauthAccountID(fromAuthJSONData: data), account.accountId)
    }

    func testOAuthAccountIDFromAuthJSONDataRejectsNonOAuthOrMalformedPayload() throws {
        let store = CodexBarConfigStore()
        let apiKeyData = try JSONSerialization.data(withJSONObject: ["OPENAI_API_KEY": "client-key"], options: [.sortedKeys])

        XCTAssertNil(store.oauthAccountID(fromAuthJSONData: apiKeyData))
        XCTAssertNil(store.oauthAccountID(fromAuthJSONData: Data("not-json".utf8)))
        XCTAssertNil(store.oauthAccountID(fromAuthJSONData: nil))
    }
    ...
    func testLoadOrMigrateKeepsTeamBadgeStableForSharedTeamOrganizationName() throws {
        let store = CodexBarConfigStore()
