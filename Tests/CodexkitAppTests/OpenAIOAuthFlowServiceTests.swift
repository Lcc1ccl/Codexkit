...
final class OpenAIOAuthFlowServiceTests: CodexBarTestCase {
    ...
    func testRefreshAccountAllowsDowngradeToFreePlanType() async throws {
        let refreshedAccessToken = try self.makeJWT(payload: [
            "exp": Date(timeIntervalSince1970: 1_780_103_600).timeIntervalSince1970,
            "client_id": "app_refresh_client",
            "https://api.openai.com/auth": [
                "chatgpt_account_id": "acct_refresh_free",
                "chatgpt_account_user_id": "acct_refresh_free",
                "chatgpt_plan_type": "free",
            ],
        ])
        let refreshedIDToken = try self.makeJWT(payload: [
            "email": "refresh-free@example.com",
            "https://api.openai.com/auth": [
                "chatgpt_subscription_active_until": "2026-01-01T00:00:00Z",
            ],
        ])
        let account = try self.makeOAuthAccount(
            accountID: "acct_refresh_free",
            email: "refresh-free@example.com",
            refreshToken: "refresh-free-old",
            planType: "plus",
            oauthClientID: "app_refresh_client"
        )
        ...
        MockURLProtocol.handler = { request in
            ...
            let data = try JSONSerialization.data(withJSONObject: [
                "access_token": refreshedAccessToken,
                "refresh_token": "refresh-free-new",
                "id_token": refreshedIDToken,
            ], options: [.sortedKeys])
            return (response, data)
        }
        ...
        XCTAssertEqual(refreshed.planType, "free")
        XCTAssertEqual(refreshed.refreshToken, "refresh-free-new")
    }
}
